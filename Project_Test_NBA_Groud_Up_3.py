...
def get_features():
    # import dataset (must be in order of picks)
    features = pd.read_csv('Data_WS_1_TNT.csv')
    
    # drop name and pick from dataset
...
# run the random forest model and report the predictions (can also optially report the importances)
def rand_forest(train_features,test_features,train_labels,new_feature_list,runs=1,n_estimators = 1000,max_depth=None,get_importances=False):
    
    sorted_importances = []
    
    # run the random forest model a certain number of times for more accuracy
    for b in range(runs):
        print(b)
        # Instantiate model with chosen paramaters
...
def rand_forest_eval_1_main(itterations=1, runs=5, max_depth=None):
    ...
    for t in range(itterations):
        ...
        predictions_matrix = rand_forest(train_features,test_features,train_labels,new_feature_list,runs,max_depth=max_depth)[0]
...
output = rand_forest_eval_1_main(30,5,2)
